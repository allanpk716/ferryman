...
**验收标准**:
- [x] 正确 token + loopback → 守护优雅退出(监听口释放)
- [x] 错 token / 无 token / GET 方法 → 401/405,守护不受影响
- [x] 端点拒绝非 loopback 来源(测试伪造 RemoteAddr)
- [x] `internal/mcp` 禁词/只读红线测试零回归(动词面无新增)
- [x] `go test ./internal/daemon/` 绿
