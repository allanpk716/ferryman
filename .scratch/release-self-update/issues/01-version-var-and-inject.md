...
**验收标准**:
- [x] 不注入时 `ferryman version` 输出 `dev`(含"非 release 构建"提示)
- [x] 注入 `vX.Y.Z` 后子命令输出该值(测试用注入变量或 ldflags 冒烟证明)
- [x] `build.ps1 -Release` 编出的 exe `version` 输出与 `git describe --tags --always` 一致(手工冒烟一次,结果记票)
- [x] `go test ./cmd/ferryman/` 绿;无参/serve 等既有子命令行为零变化
- [x] 验证用的临时 exe 一律 `-o $env:TEMP\...`,不碰仓库根 `ferryman.exe`
