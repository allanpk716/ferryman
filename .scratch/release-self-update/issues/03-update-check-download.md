...
**验收标准**:
- [x] httptest 伪 GitHub:latest 排除 prerelease;`--prerelease` 能见 rc;显式版本(含更低版本)能解析
- [x] 下载经 `HTTPS_PROXY` 环境变量生效(测试用本地代理桩或 httptest 证明走了代理 URL)
- [x] SHA256 不符返回明确错误且不留半文件
- [x] `--check` 输出人话(已是最新 vX.Y.Z / 发现 vX.Y.Z,当前 vX.Y.Z)
- [x] `go test ./internal/update/ ./cmd/ferryman/` 绿;不触碰 `internal/mcp`
