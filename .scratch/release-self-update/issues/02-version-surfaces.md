...
**验收标准**:
- [x] doctor 输出含版本行(dev 构建显 dev)
- [x] MCP doctor 响应 JSON 含 `version` 字段;`internal/mcp` 全部既有测试(含禁词/只读红线)零回归、零动词面变化
- [x] `/stats` 响应含 `version`;既有 /stats 消费方(看门探活、面板)不受影响
- [x] 面板页脚显示版本号;前端遵守既有纪律(账本数据只经 createElement/textContent 进 DOM)
- [x] `go test ./internal/installer/ ./internal/mcp/ ./internal/daemon/ ./cmd/ferryman/` 绿
