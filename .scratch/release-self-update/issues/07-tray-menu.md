...
**验收标准**:
- [x] 菜单项齐、顺序对、版本项 disabled 且随版本变量显示
- [x] 「检查更新」不发写操作、结果可感知(notify 落地证据或气泡调用证据)
- [x] 「立即升级」以 detached 隐藏方式拉起监督者,守护自身不退出不卡 UI(菜单结构用测试断言;实际升级链路归票05 测试)
- [x] `--no-tray` 模式零影响;`go test ./cmd/ferryman/` 绿
