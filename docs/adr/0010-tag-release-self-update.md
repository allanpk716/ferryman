...
- **自中继(实施补,v0.1.0 首发实测)**:`ferryman update` 的监督者进程自身往往就跑在换装目标 exe 里——停旧守护只停了另一个进程,监督者自己的运行镜像仍锁着文件,单次 `MoveFileEx(REPLACE_EXISTING)` 实测报 Access is denied(v0.1.0 升级演练实证;票05 测试用替身守护与监督者分属两文件,未覆盖此生产拓扑)。修法:监督者发现自身映像 == 换装目标时,复制自身为 `<目标>.supervisor-copy`、detached 拉起副本携 `--self-relay` 接手,本进程交棒——副本与目标不同文件,原子替换恢复可行;副本无法删除自身运行镜像,`.supervisor-copy*` 入残留清扫域,下次 update/doctor 收走。回滚链路在该次实测中已按设计工作(旧版恢复服务、诚实报错)。
