...
- CC 2.1.232（不升级）；Codex CLI 0.153.0。CC 会话 `~/.claude/projects/<转义>/<id>.jsonl`（1644 个/1.1GB）；Codex `~/.codex/sessions/年/月/日/rollout-*.jsonl`（317 个/93MB）。
...
- 通知复用 claude-notify（Pushover + Toast）。
