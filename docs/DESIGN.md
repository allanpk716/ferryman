...
不做无条件全局保活——原"不保活"红线针对的是 ferry 无差别续命所有会话的全局 keepalive，此项仍禁；条件化、会话级、短窗的保温（问询守望心跳：仅提问潮命中会话，默认至多两跳，窗口关即止）经 [ADR-0004](adr/0004-conditioned-session-keepalive.md) 显式放行，`[question_watch]` 独立配置、默认 off、三态开关永不自动升级。其余红线不变：不抢 5 分钟缓存窗；不碰运行中会话；不嵌 Orca；不升级 CC。
...
15. §9 红线"不保活"改写为"不做无条件全局保活"：红线原意针对无条件全局 keepalive；条件化、会话级、短窗保温（问询守望）经 ADR-0004 放行，默认关、三态永不自动升级（§9）。
