...
// InjectOpen/InjectClose INJECT 层标记（ferry.py:25 INJECT_OPEN/INJECT_CLOSE
// 同值占位）。票18 ferry 执行器接真源时，此处改引其单源（勿散抄第二份）。
const (
	InjectOpen  = "<<<INJECT>>>"
	InjectClose = "<<</INJECT>>>"
)
...
func (d *Daemon) Restore(agent, cwd, sessionID string) map[string]any {
	...
	var inject string
	if strings.Contains(md, InjectOpen) && strings.Contains(md, InjectClose) {
		// Python md.split(INJECT_OPEN,1)[1].split(INJECT_CLOSE,1)[0].strip()
		inject = strings.TrimSpace(
			strings.SplitN(strings.SplitN(md, InjectOpen, 2)[1], InjectClose, 2)[0])
	} else {
