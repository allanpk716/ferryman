...
- 生成:2026-09-18 18:14(工具:`ferryman e0c`)
- 数据:293 个会话(终值 288,在跑 5),扫描根:C:\Users\allan716\.claude\projects
...
| 行 | 类型 | 深度 | 模型 | 终态 | 响应 | input | output | 缓存写 | 缓存读 | subtree(input/output/缓存写/缓存读) |
|---|---|---|---|---|---:|---:|---:|---:|---:|---|
| 主会话 | — | — | GLM-5.3 | 在跑(非终值) | 39 | 106,686 | 57,590 | 0 | 3,466,176 | — |
| `a0548fa2192a85f9b` | general-purpose | 1 | haiku | 在跑(非终值) | 1 | 5,941 | 165 | 0 | 23,936 | 5,941 / 165 / 0 / 23,936 |
...
| `abdc17f3474c32bc7` | general-purpose | 1 | haiku | 在跑(非终值) | 2 | 37,833 | 861 | 0 | 24,000 | 37,833 / 861 / 0 / 24,000 |
| `aca2aa923970c3472` | general-purpose | 1 | haiku | 完成 | 4 | 60,961 | 2,849 | 0 | 69,696 | 60,961 / 2,849 / 0 / 69,696 |

- 会话总账(主 + Σ 子 self,含嵌套不双计):input 293,243 / output 68,803 / 缓存写 0 / 缓存读 3,912,960
- 交叉校验:direct 口径:主转录 Agent/Task 调用 5 次 / depth=1 meta 5 条 / depth=1 转录 5 份 — 一致
- 交叉校验:total 口径:各父转录直接子调用合计 5 次 / total spawn 事件 5 次(未知深度 0 条)— 一致
...
| 行 | 类型 | 深度 | 模型 | 终态 | 响应 | input | output | 缓存写 | 缓存读 | subtree(input/output/缓存写/缓存读) |
|---|---|---|---|---|---:|---:|---:|---:|---:|---|
| 主会话 | — | — | GLM-5.3 | 在跑(非终值) | 64 | 183,009 | 115,022 | 0 | 8,376,384 | — |
...
| `a94d95daf9fa2bd9f` | general-purpose | 1 | sonnet | 在跑(非终值) | 15 | 144,074 | 27,022 | 0 | 715,520 | 144,074 / 27,022 / 0 / 715,520 |
| `abd127490155e178f` | general-purpose | 1 | haiku | 完成 | 5 | 42,629 | 2,837 | 0 | 125,952 | 42,629 / 2,837 / 0 / 125,952 |
| `af70ed39d2e089a07` | general-purpose | 1 | sonnet | 在跑(非终值) | 6 | 46,046 | 1,297 | 0 | 195,264 | 46,046 / 1,297 / 0 / 195,264 |

- 会话总账(主 + Σ 子 self,含嵌套不双计):input 671,233 / output 164,855 / 缓存写 0 / 缓存读 10,520,064
- 交叉校验:direct 口径:主转录 Agent/Task 调用 7 次 / depth=1 meta 7 条 / depth=1 转录 7 份 — 一致
- 交叉校验:total 口径:各父转录直接子调用合计 7 次 / total spawn 事件 7 次(未知深度 0 条)— 一致
...
| 行 | 类型 | 深度 | 模型 | 终态 | 响应 | input | output | 缓存写 | 缓存读 | subtree(input/output/缓存写/缓存读) |
|---|---|---|---|---|---:|---:|---:|---:|---:|---|
| 主会话 | — | — | GLM-5.3 | 在跑(非终值) | 122 | 416,851 | 119,737 | 0 | 17,898,816 | — |
...
| `a47b276918310cb6c` | general-purpose | 1 | sonnet | 在跑(非终值) | 59 | 295,291 | 53,073 | 0 | 5,447,744 | 295,291 / 53,073 / 0 / 5,447,744 |
| `a480bd46d3d2f1738` | general-purpose | 1 | sonnet | 完成 | 9 | 75,832 | 21,797 | 0 | 479,424 | 75,832 / 21,797 / 0 / 479,424 |
...
- 会话总账(主 + Σ 子 self,含嵌套不双计):input 2,087,964 / output 427,369 / 缓存写 0 / 缓存读 35,863,488
- 交叉校验:direct 口径:主转录 Agent/Task 调用 14 次 / depth=1 meta 14 条 / depth=1 转录 14 份 — 一致
- 交叉校验:total 口径:各父转录直接子调用合计 14 次 / total spawn 事件 14 次(未知深度 0 条)— 一致
...
| 行 | 类型 | 深度 | 模型 | 终态 | 响应 | input | output | 缓存写 | 缓存读 | subtree(input/output/缓存写/缓存读) |
|---|---|---|---|---|---:|---:|---:|---:|---:|---|
| 主会话 | — | — | GLM-5.3 | 在跑(非终值) | 72 | 442,807 | 93,373 | 0 | 12,690,816 | — |
...
| `adeaf77f867d1fe5a` | general-purpose | 1 | sonnet | 在跑(非终值) | 20 | 114,055 | 18,090 | 0 | 1,575,232 | 114,055 / 18,090 / 0 / 1,575,232 |
...
- 会话总账(主 + Σ 子 self,含嵌套不双计):input 1,233,828 / output 197,410 / 缓存写 0 / 缓存读 24,484,992
- 交叉校验:direct 口径:主转录 Agent/Task 调用 7 次 / depth=1 meta 7 条 / depth=1 转录 7 份 — 一致
- 交叉校验:total 口径:各父转录直接子调用合计 7 次 / total spawn 事件 7 次(未知深度 0 条)— 一致
...
| 行 | 类型 | 深度 | 模型 | 终态 | 响应 | input | output | 缓存写 | 缓存读 | subtree(input/output/缓存写/缓存读) |
|---|---|---|---|---|---:|---:|---:|---:|---:|---|
| 主会话 | — | — | GLM-5.3 | 在跑(非终值) | 98 | 833,546 | 149,796 | 0 | 19,151,168 | — |
...
| `a533873e75125c40a` | general-purpose | 1 | sonnet | 在跑(非终值) | 25 | 197,248 | 23,169 | 0 | 1,921,216 | 197,248 / 23,169 / 0 / 1,921,216 |
...
| `af111e193343d8820` | general-purpose | 1 | opus | 在跑(非终值) | 26 | 117,027 | 27,001 | 0 | 2,177,088 | 117,027 / 27,001 / 0 / 2,177,088 |
...
- 会话总账(主 + Σ 子 self,含嵌套不双计):input 2,077,498 / output 405,781 / 缓存写 0 / 缓存读 32,843,648
- 交叉校验:direct 口径:主转录 Agent/Task 调用 15 次 / depth=1 meta 15 条 / depth=1 转录 15 份 — 一致
- 交叉校验:total 口径:各父转录直接子调用合计 15 次 / total spawn 事件 15 次(未知深度 0 条)— 一致
...
|---|---:|---:|---:|---:|
| 主会话合计 | 191,159,381 | 26,232,628 | 213,487 | 6,672,861,971 |
| 子代理合计(self) | 100,109,224 | 23,310,568 | 0 | 1,983,580,160 |
| 总账 | 291,268,605 | 49,543,196 | 213,487 | 8,656,442,131 |
...
|---|---:|---:|---:|---:|
| GLM-5.3 | 36,206,322 | 5,695,005 | 0 | 993,315,072 |
...
- 手记:`C:/Users/allan716/orca/workspaces/Ferryman/subagent监控/.xcheck/20260918-151538/recon-notes.jsonl`(有效 18 条,坏行 0 条)
- 残差口径=in+out(与 CLI 自报语义对齐;四列口径另列仅作参考)。

| agentId | 类型 | 自报总量 | 文件四列合计(参考) | 文件 in+out | 残差(in+out) | 残差率 | 备注 |
|---|---|---:|---:|---:|---:|---:|---|
| `a1ec147dd4f7a81f0` | Explore | 75,395 | 762,208 | 74,016 | 1,379 | +1.8% | |
| `a84478bc85bff9c18` | carrier(codex r0) | 39,776 | 185,258 | 41,322 | -1,546 | -3.9% | |
| `ae403b8e672237d17` | carrier(pi r0) | 40,138 | 223,817 | 41,993 | -1,855 | -4.6% | |
| `afe934b46d0893568` | carrier(codex r1) | 40,413 | 187,133 | 47,101 | -6,688 | -16.5% | |
| `a4cc2e7381a796c28` | carrier(pi r1) | 40,494 | 187,798 | 39,126 | 1,368 | +3.4% | |
| `a20df93958b185b64` | implementer(ticket01) | 64,377 | 961,271 | 98,231 | -33,854 | -52.6% | |
| `ac0a930fdf75c0695` | reviewer(ticket01) | 70,809 | 530,534 | 87,782 | -16,973 | -24.0% | |
| `a779dceb0c97e33bd` | implementer(ticket02) | 85,242 | 2,141,325 | 281,101 | -195,859 | -229.8% | |
| `acf6a35775f7aa1e4` | reviewer(ticket02) | 78,654 | 1,277,626 | 184,634 | -105,980 | -134.7% | |
| `a779dceb0c97e33bd` | implementer(ticket02 R1) | 96,294 | 2,141,325 | 281,101 | -184,807 | -191.9% | |
| `acf6a35775f7aa1e4` | reviewer(ticket02 re-review) | 87,243 | 1,277,626 | 184,634 | -97,391 | -111.6% | |
| `ab24ad7f24ade4808` | implementer(ticket03) | 103,997 | 3,837,889 | 297,217 | -193,220 | -185.8% | |
| `ab2816da299c7a3a3` | reviewer(ticket03) | 86,441 | 1,442,848 | 213,792 | -127,351 | -147.3% | |
| `ab24ad7f24ade4808` | implementer(ticket03 R1) | 110,749 | 3,837,889 | 297,217 | -186,468 | -168.4% | |
| `ab2816da299c7a3a3` | reviewer(ticket03 re-review) | 91,602 | 1,442,848 | 213,792 | -122,190 | -133.4% | |
| `a47b276918310cb6c` | implementer(ticket04) | 110,388 | 5,796,108 | 348,364 | -237,976 | -215.6% | |
| `a480bd46d3d2f1738` | reviewer(ticket04) | 91,299 | 577,053 | 97,629 | -6,330 | -6.9% | |
| `a9e43d99db3a953cd` | implementer(ticket05) | 102,868 | 1,832,549 | 126,437 | -23,569 | -22.9% | |
...
| 类别 | 数量 | 分母 | 占比 |
|---|---:|---:|---:|
| 中断(子代理终态) | 1 | 1499(子代理行) | 0.1% |
| 空文件(子代理终态) | 0 | 1499(子代理行) | 0.0% |
| 在跑(子代理,非终值) | 8 | 1499(子代理行) | 0.5% |
| 在跑会话(整场非终值) | 5 | 293(会话) | 1.7% |
| 有 meta 无转录(不成对) | 0 | 1499(子代理行) | 0.0% |
| 有转录无 meta(未知桶,不成对) | 0 | 1499(子代理行) | 0.0% |
| 无 message.id 行 | 0 | 1792(转录文件) | 0.0% |
| 坏行/半行 JSON | 0 | 1792(转录文件) | 0.0% |
| 组内非零 usage 冲突 | 0 | 1792(转录文件) | 0.0% |
| 全零占位组 | 811 | 1792(转录文件) | 45.3% |
...
- `fd6159b7-f571-44d8-bafa-cec79e122498`:
  - agent a0548fa2192a85f9b:message.id=msg_2026091818132090c3c3af78744a63:全零占位组(1 行),无实际 usage,不计入
...
- 判据一(口径可信):Q3 残差率(in+out 口径)多数 ≤5% 且无 >10% 离群 → 文件侧 in+out 口径可作对账权威;出现 >10% 离群先解释再议(spec 已知残差未解释,见 Q3)。本机实测:残差率最大 229.8%(n=18)。
- 判据二(功能价值):若子代理占比可观(参考:output 列 ≥20%)且会话平均 spawn 数不小(本机实测:终值会话平均 5.0 次/会话,计 1,451 次),记账入正式功能才有信息收益;两值都低则留实验记录即可。
