#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""用 pi agent 实测智谱 Coding Plan 缓存 TTL（Track B 自动化）。

为什么走 pi：Coding Plan 端点有"仅限指定工具"门禁，裸调 API 会 401；
pi 是真实 agent 工具链路（默认经 ccswitch 本地代理 -> GLM-5.3），官方放行。

原理与 Track A 相同：每个等待档位一条独立 pi 会话（独立长资料），
写入 -> 干等 -> resume 探一次，从 session 文件读 usage.cacheRead 判命中。
资料经 --append-system-prompt 注入系统提示（两次调用同一文件，字节级一致）。

用法：
  # 单臂冒烟（12 秒间隔、2k 字符，验证链路 + 解析）
  python pi_cache_probe.py arm --gap 0.2 --chars 2000

  # 正式阶梯（默认 1,3,5,8,12,20,30,45,60,90,120 分钟 x3，总墙钟约 2.5 小时）
  python pi_cache_probe.py ladder

  # 汇总（与 Track A 的 report 字段兼容，也可用 cache_ttl_probe.py report）
  python pi_cache_probe.py report --results results/pi_*.jsonl
"""
...
from cache_ttl_probe import HIT_RATIO, build_prefix, now_iso, sleep_until

HERE = Path(__file__).resolve().parent
DEFAULT_PROVIDER = "ccswitch"          # 本机经 CC Switch 代理转发 GLM-5.3
DEFAULT_MODEL = "claude-opus-4-8"       # CC Switch 侧映射到 GLM-5.3
...
def run_pi(pi: str, session_dir: Path, material: Path, prompt: str,
           provider: str, model: str, session_file: str | None = None) -> str:
    """跑一发 pi 非交互调用，返回 stdout（出错时抛异常带 stderr 摘要）。"""
    cmd = [pi, "-p", *PI_FLAGS, "--provider", provider, "--model", model,
...
def parse_session_usage(session_file: Path) -> list[dict]:
    """从 pi session JSONL 里按顺序抽出每条 assistant 消息的 usage。"""
    out = []
...
def write_arm(pi: str, arm_id: str, chars: int, provider: str, model: str,
              results: Path, state: dict) -> None:
    ...
    record(results, phase="ladder", arm_id=arm_id, gap_min=st.get("gap_min"),
           event="write", provider=provider, model=model, **(st["write_usage"] or {}))
    print(f"[write] {arm_id}: usage={st['write_usage']}")


def probe_arm(pi: str, arm_id: str, gap_min: float, provider: str, model: str,
              results: Path, state: dict) -> None:
    """等到 written_at+gap 后 resume 探测一次，解析 cacheRead 判命中。"""
    st = state[arm_id]
    deadline = st["written_at"] + gap_min * 60
    wait = (deadline - time.time()) / 60
    print(f"[wait ] {arm_id}: 还需 {max(wait, 0):.1f} 分钟后探测")
...
def main() -> None:
    ...
    p.add_argument("--tag", default=None, help="臂名前缀（默认 auto）")
    ...
    p = sub.add_parser("report", help="汇总 JSONL（字段兼容 Track A）")
    ...
    if a.mode == "report":
        from cache_ttl_probe import cmd_report as report
        rep = argparse.Namespace(results=a.results)
        report(rep)
        return
    ...
    try:
        ...
        print("\n已中断并保存进度，重跑同命令可续跑。")
        return
    print(f"\n完成。汇总：python pi_cache_probe.py report --results \"{out_dir}/pi_ladder_*.jsonl\"")
