#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""缓存 TTL 探针 —— API 轨（任意 OpenAI 兼容端点直打）
...
原理：发一段固定长前缀 -> 等 X 分钟 -> 再发几乎相同的请求 ->
      看响应 usage 里的缓存命中文本数（满值=活着，0=已失效）。
      每个等待档位用一段全新前缀（写入 -> 干等 -> 只探一次），避免探测本身给缓存续命。

cached token 字段兼容：prompt_tokens_details.cached_tokens（OpenAI/Kimi/Qwen）、
prompt_cache_hit_tokens（DeepSeek）、cache_read_input_tokens（Anthropic 风格）。
有工具门禁的订阅套餐请改用 agent 轨 probe_pi.py。

用法（详见同目录 README.md 的服务商配方）：
  python probe_api.py arm     --gap 0.2 --chars 2000 --base-url ... --model ... --api-key ...
  python probe_api.py preflight --base-url ... --model ... --api-key ...
  python probe_api.py ladder  --base-url ... --model ... --api-key ...
  python probe_api.py refresh --base-url ... --model ... --api-key ... --interval 8 --count 6
  python probe_api.py report  --results results/
...
DEFAULT_UA = "cache-ttl-probe/1.0"
WRITE_USER = "请记住以上资料备用。请只回复：READY"
PROBE_USER = "请只回复：OK"
HIT_RATIO = 0.85  # cached/prompt >= 该值记为命中（容忍尾部新增 token）
MAX_TOKENS = 16
TIMEOUT_S = 240
...
def extract_cached(usage: dict):
    """多服务商缓存命中字段别名。返回命中 token 数或 None（字段缺失）。"""
    ptd = usage.get("prompt_tokens_details") or {}
    for v in (ptd.get("cached_tokens"),           # OpenAI / Kimi / Qwen 兼容层
              usage.get("prompt_cache_hit_tokens"),  # DeepSeek
              usage.get("cached_tokens"),
              usage.get("cache_read_input_tokens")):  # Anthropic 风格
        if v is not None:
            return v
    return None
...
def post_chat(base_url: str, api_key: str, ua: str, model: str, messages: list,
    ...
    for attempt in range(1, 4):  # 网络错误/429 重试，4xx/5xx 业务错误不重试
        ...
        try:
            ...
            cached = extract_cached(usage)
            return {"ok": True, "http": resp.status,
                    "latency_ms": round((time.monotonic() - t0) * 1000),
                    "prompt_tokens": usage.get("prompt_tokens"),
                    "completion_tokens": usage.get("completion_tokens"),
                    "cached_tokens": cached,
                    "has_cache_field": cached is not None,
                    "usage_raw": usage, "attempt": attempt}
...
def cmd_preflight(a) -> None:
    """两发小请求：验证 key/端点可达 + usage 里有缓存信号；API 轨还要人工核对计费路径。"""
    ...
    for label in ("write", "probe"):
        r = post_chat(a.base_url, a.api_key, a.user_agent, a.model,
                      make_messages(prefix, PROBE_USER), extra_body=a.extra_body)
    ...
    print(f"\n结果已写入 {results}")
    print("检查点：1) 第二发 cached_tokens > 0（有缓存信号，否则换轨/查字段）；"
          "2) 控制台费用明细走的是预期计费路径（订阅/标准 API/资源包）。")
...
def cmd_ladder(a) -> None:
    ...
    for arm_id in todo:
        arm = arms[arm_id]
        r = post_chat(a.base_url, a.api_key, a.user_agent, a.model,
                      make_messages(arm["prefix"], WRITE_USER), extra_body=a.extra_body)
        arm["write"] = r
        if r["ok"]:
            arm["written_at"] = time.time()
        record(results, phase="ladder", arm_id=arm_id, gap_min=arm["gap_min"],
               event="write", model=a.model, ua=a.user_agent,
               **{k: v for k, v in r.items() if k != "usage_raw"})
    ...
    try:
        while True:
            ...
            r = post_chat(a.base_url, a.api_key, a.user_agent, a.model,
                          make_messages(arm["prefix"], PROBE_USER), extra_body=a.extra_body)
        ...
        return
    print(f"\n全部臂完成。汇总：python probe_api.py report --results \"{out_dir}\"")
...
def cmd_refresh(a) -> None:
    ...
    r = post_chat(a.base_url, a.api_key, a.user_agent, a.model,
                  make_messages(prefix, WRITE_USER), extra_body=a.extra_body)
    record(results, phase="refresh", arm_id=arm_id, event="write",
           model=a.model, **{k: v for k, v in r.items() if k != "usage_raw"})
    ...
    try:
        for k in range(1, a.count + 1):
            ...
            record(results, phase="refresh", arm_id=arm_id, event="probe",
                   offset_min=round(offset, 2), model=a.model,
                   **{k2: v for k2, v in r.items() if k2 != "usage_raw"})
            ratio = (r.get("cached_tokens") / r["prompt_tokens"]
                     if r["ok"] and r.get("prompt_tokens") else None)
...
def main() -> None:
    ...
    def common(p):
        ...
        p.add_argument("--user-agent", default=DEFAULT_UA,
                       help="有工具门禁的服务商在此伪装受支持工具")
        p.add_argument("--thinking-off", action="store_true",
                       help="附加 thinking={type:disabled}（GLM 系等思考型模型省输出）")
    ...
    p = sub.add_parser("preflight", help="门禁/计费/信号预检：两发小请求")
    ...
    p.set_defaults(fn=lambda a: _cmd_arm(a))
    ...
    p = sub.add_parser("report", help="汇总 results/ 下的 JSONL（与 agent 轨字段兼容）")
    ...
    if a.cmd != "report":
        ...
        a.extra_body = {"thinking": {"type": "disabled"}} if a.thinking_off else {}
        if not a.api_key:
            sys.exit("缺少 API key：--api-key 或环境变量 ZHIPU_API_KEY")
        if not a.base_url or not a.model:
            sys.exit("API 轨需要 --base-url 和 --model（配方见 README.md）")
    a.fn(a)


def _cmd_arm(a) -> None:
    """单臂冒烟：小前缀 + 短间隔，验证链路与缓存信号。"""
    out_dir = Path(a.out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    results = out_dir / f"smoke_{datetime.now():%Y%m%d_%H%M%S}.jsonl"
    arm_id = a.tag or f"smoke-{int(time.time())}"
    prefix = build_prefix(arm_id, a.chars)
    r1 = post_chat(a.base_url, a.api_key, a.user_agent, a.model,
                   make_messages(prefix, WRITE_USER), extra_body=a.extra_body)
    record(results, phase="smoke", arm_id=arm_id, event="write", model=a.model, **r1)
    print(f"[write] ok={r1['ok']} prompt={r1.get('prompt_tokens')} "
          f"cached={r1.get('cached_tokens')} err={r1.get('error')}")
    sleep_until(time.time() + a.gap * 60)
    r2 = post_chat(a.base_url, a.api_key, a.user_agent, a.model,
                   make_messages(prefix, PROBE_USER), extra_body=a.extra_body)
    record(results, phase="smoke", arm_id=arm_id, event="probe", model=a.model, **r2)
    ratio = (r2.get("cached_tokens") / r2["prompt_tokens"]
             if r2["ok"] and r2.get("prompt_tokens") else None)
    print(f"[probe] cacheRead 类字段={r2.get('cached_tokens')}/{r2.get('prompt_tokens')} "
          f"({'HIT' if ratio is not None and ratio >= HIT_RATIO else 'MISS' if ratio is not None else '无字段'})"
          f"  -> {results}")
    if ratio is None:
        print("警告：响应里没有已知的缓存命中字段，先排查字段映射再跑阶梯（见 README「信号先行」）")
